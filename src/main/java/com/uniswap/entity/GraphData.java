...
public class GraphData {
    ...
    private List<String> baseTokenList = new ArrayList<>();

    public void setBaseTokenList(List<String> baseTokenList) {
        this.baseTokenList = baseTokenList;
    }
    ...
    public void addBscEdge(PairBscEntity pairBcsEntity) {
        graph.addBscEdge(pairBcsEntity);
    }
     ...
     * 构建全量Token图
     */
    public void buildDigraph(List<PairEntity> pairEntityList) {
        graph = new Graph();
     ...
     * 构建全量Token图
     */
    public void buildBscDigraph(List<PairBscEntity> pairEntityList) {
        graph = new Graph();
        pairEntityList.forEach(pairEntity -> graph.addBscEdge(pairEntity));
        baseGraph = new Graph();
        //根据基础Token信息构建基础Token图
        List<PairBscEntity> basePairEntityList = pairEntityList.stream()
                .filter(pairEntity -> baseTokenList.contains(pairEntity.getToken0()) || baseTokenList.contains(pairEntity.getToken1()))
                .collect(Collectors.toList());
        basePairEntityList.forEach(pairEntity -> baseGraph.addBscEdge(pairEntity));
    }
    ...
    public static void main(String[] args) {
        ...
        graphData.buildDigraph(tokenList);
        //[["美元","人民币"],["美元","欧元","人民币"],["美元","欧元","英镑","人民币"],["美元","英镑","人民币"],["美元","英镑","日元","人民币"]]
        System.out.println(JSONObject.toJSONString(graphData.searchPath(token0, token1)));
    }
    ...
    private static class Graph {
        ...
        public void addBscEdge(PairBscEntity pairBscEntity) {
            int start = getIndexAndSet(pairBscEntity.getToken0());
            int end = getIndexAndSet(pairBscEntity.getToken1());
            //System.out.println("start=" + start + ",end=" + end);
            addEdge(start, end);
        }
